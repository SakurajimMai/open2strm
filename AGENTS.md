...
```
修改下载流程或 cron 调度后建议用小范围源目录做一次烟雾测试，检查任务详情、进度字段、`downloads/`、定时规则状态和数据库错误日志。本地可自建 `tests/` 做回归验证，但测试文件不进入正式仓库。
...
## 验证指南
正式仓库不提交测试文件。需要本地回归时可使用 Python `unittest`，测试文件临时放在 `tests/`，命名采用 `test_*.py`；提交前确认 `tests/` 仍被忽略。涉及真实 OpenList 的流程保留为小范围烟雾测试。

## 提交与合并请求
建议使用简短的祈使句提交信息，例如 `fix retry backoff` 或 `add proxy support`。PR 需要说明变更目的、影响范围和验证方式；若涉及配置或输出格式变化，请附上示例配置片段或关键日志。不要提交 `config.yaml`、`downloads/`、`data/`、日志文件或 `tests/`。
